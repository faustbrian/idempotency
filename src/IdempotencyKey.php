...
use Cline\Idempotency\Exceptions\HexToBinaryConversionFailedException;
use Cline\Idempotency\Exceptions\InvalidHashInVersionedStringException;
use Cline\Idempotency\Exceptions\InvalidTruncateLengthException;
use Cline\Idempotency\Exceptions\InvalidVersionedKeyFormatException;
use Cline\Idempotency\Exceptions\InvalidVersionPrefixException;
use Cline\Idempotency\Exceptions\UnsupportedVersionException;
use Stringable;
...
final readonly class IdempotencyKey implements Stringable
{
     ...
     *
     * @throws InvalidHashInVersionedStringException If the hash is invalid
     * @throws InvalidVersionedKeyFormatException    If the string format is invalid
     * @throws InvalidVersionPrefixException         If version doesn't start with 'v'
     * @throws UnsupportedVersionException           If version is not supported
     *
    ...
    public static function fromVersionedString(string $versionedString): self
    {
        ...
        throw_if(count($parts) !== 3, InvalidVersionedKeyFormatException::invalidFormat());

        [$version, $algorithmName, $hash] = $parts;

        throw_unless(str_starts_with($version, 'v'), InvalidVersionPrefixException::mustStartWithV());

        $versionNumber = (int) mb_substr($version, 1);

        if ($versionNumber !== self::VERSION) {
            throw UnsupportedVersionException::forVersion($versionNumber);
        }

        $algorithm = HashAlgorithm::fromString($algorithmName);

        throw_unless(self::isValid($hash, $algorithm), InvalidHashInVersionedStringException::invalid());
     ...
     *
     * @throws HexToBinaryConversionFailedException If the hex to binary conversion fails
     *
    ...
    public function toBinary(): string
    {
        ...
        throw_if($binary === false, HexToBinaryConversionFailedException::failed());
     ...
     *
     * @throws InvalidTruncateLengthException If length is not greater than 0
     *
     * @return string The truncated hash
     */
    public function truncate(int $length): string
    {
        throw_if($length <= 0, InvalidTruncateLengthException::mustBeGreaterThanZero());
